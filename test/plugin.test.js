...
describe('=== transform import declaration correctly ===', () => {
    ...
    it('with default options', () => {
        ...
        expect(result.includes('import Button from \'chimera-ui/lib/components/Button\'')).toBeTruthy();
        expect(result.includes('import \'chimera-ui/lib/components/Button/style/index.scss\'')).toBeTruthy();
    });

    it('with css style', () => {
        const result = babel(code, { style: 'css' });
        expect(result.includes('import Button from \'chimera-ui/lib/components/Button\'')).toBeTruthy();
        expect(result.includes('import \'chimera-ui/lib/components/Button/style/css/index.css\'')).toBeTruthy();
    });

    it('without style', () => {
        const result = babel(code, { style: false });
        expect(result.includes('import Button from \'chimera-ui/lib/components/Button\'')).toBeTruthy();
        expect(result.includes('import \'chimera-ui/lib/components/Button/style/index.scss\'')).toBeFalsy();
    });

    it('modify libDirectory', () => {
        expect(babel(code, { libDirectory: 'xxx' }).includes('import Button from \'chimera-ui/xxx/components/Button\'')).toBeTruthy();
    });
